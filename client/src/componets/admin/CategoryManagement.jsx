...
const CategoryManagement = () => {
  ...
  const handleRemoveSubcategory = (subjectIndex, subcatIndex) => {
    const updated = [...newCategory.subjects];
    updated[subjectIndex].subcategories = updated[subjectIndex].subcategories.filter(
      (_, i) => i !== subcatIndex
    );
    setNewCategory({ ...newCategory, subjects: updated });
  };
  ...
  const handleCreateCategory = async () => {
    ...
    const validSubjects = newCategory.subjects.filter((s) => s.name.trim() !== "");
  ...
  const renderSubjectForm = (subject, index, isEdit = false) => {
    ...
    return (
      <div key={index} className="border border-white/10 rounded-xl p-4 bg-black/20">
        <div className="flex gap-2 mb-3">
          <input
            ...
            className="flex-1 bg-black/30 border border-white/10 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-emerald-500/50"
          />
          {handlers.subjects.length > 1 && (
            <button
              onClick={() => handlers.onRemoveSubject(index)}
              className="px-4 bg-red-500/10 border border-red-500/20 text-red-400 rounded-xl hover:bg-red-500/20"
            >
              <Trash2 size={16} />
            </button>
          )}
          ...
          {subject.subcategories.map((subcat, subcatIdx) => (
            <div key={subcatIdx} className="flex gap-2">
              ...
              <input
                type="text"
                value={subcat}
                onChange={(e) => handlers.onSubcategoryChange(index, subcatIdx, e.target.value)}
                placeholder="e.g., Inorganic, Organic, Physical"
                className="flex-1 bg-black/40 border border-white/5 text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-emerald-500/50"
              />
              <button
                onClick={() => handlers.onRemoveSubcategory(index, subcatIdx)}
                className="px-3 bg-red-500/10 border border-red-500/20 text-red-400 rounded-lg hover:bg-red-500/20"
              >
                <X size={14} />
              </button>
            </div>
    ...
    <div className="min-h-screen text-white p-6 md:p-8">
      ...
      >
        ...
        <button
          ...
          className="mb-6 flex items-center gap-2 px-6 py-3 bg-linear-to-r from-emerald-600 to-teal-600 rounded-xl hover:shadow-[0_0_20px_rgba(16,185,129,0.4)] transition-all font-semibold"
        >
          <Plus size={20} />
          Create New Category
        </button>
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-black/30 border border-white/10 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-emerald-500/50"
                  />
                </div>
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  disabled={loading}
                  className="flex-1 py-3 bg-linear-to-r from-emerald-600 to-teal-600 rounded-xl hover:shadow-[0_0_20px_rgba(16,185,129,0.4)] transition-all font-semibold disabled:opacity-50"
                >
                  {loading ? (
            ...
            categories.map((category) => {
              ...
              return (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 hover:border-emerald-500/30 transition-all"
                >
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="p-2 bg-white/5 border border-white/10 rounded-lg hover:bg-white/10 text-gray-400 hover:text-emerald-400 transition-all"
                      >
                        <Edit2 size={16} />
                      </button>
                      <button
                        onClick={() => handleDeleteCategory(category.id)}
                        className="p-2 bg-red-500/10 border border-red-500/20 rounded-lg hover:bg-red-500/20 text-red-400 transition-all"
                      >
                        <Trash2 size={16} />
                      </button>
                    </div>
                      ...
                      {normalizedSubjects.map((subject, idx) => (
                        <div key={idx}>
                          <div className="flex items-center gap-2">
                            <span className="px-3 py-1.5 bg-emerald-500/10 border border-emerald-500/20 rounded-lg text-sm text-emerald-400 flex items-center gap-1 flex-1">
                              <ChevronRight size={14} />
                              {subject.name}
                            </span>
                            {subject.subcategories && subject.subcategories.length > 0 && (
                              <button
                                onClick={() => toggleSubject(category.id, idx)}
                                className="p-1.5 bg-white/5 border border-white/10 rounded-lg hover:bg-white/10 text-gray-400 transition-all"
                              >
                                <ChevronDown 
                                  size={16} 
                                  className={`transition-transform ${
                                    expandedSubjects[`${category.id}-${idx}`] ? 'rotate-180' : ''
                                  }`}
                                />
                              </button>
                            )}
                             ...
                             subject.subcategories.length > 0 && (
                              ...
                              >
                                {subject.subcategories.map((subcat, subcatIdx) => (
                                  <span
                                    key={subcatIdx}
                                    className="px-2 py-1 bg-teal-500/10 border border-teal-500/20 rounded-md text-xs text-teal-400 flex items-center gap-1"
                                  >
